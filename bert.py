...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    S = torch.matmul(query, key.transpose(-2, -1)) / torch.sqrt(torch.tensor(attention_head_size).to(key.device))
    S = S + attention_mask
    attn_scores = F.softmax(S, dim=-1)
    attn_output =  torch.matmul(attn_scores, value)
    attn_output =  attn_output.transpose(1, 2).contiguous().view(bs, seq_len, num_attention_heads * attention_head_size)
    return attn_output
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    ff_output = self.interm_af(self.interm_dense(attn_output))
    ff_output = self.add_norm(attn_output, ff_output, self.out_dense, self.out_dropout, self.out_layer_norm)
    return ff_output
